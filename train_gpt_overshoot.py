...
class Muon(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        overshoot: Whether to use overshoot momentum in the internal SGD. (recommended)
        ns_steps: The number of Newton-Schulz iteration steps to use.
    """
    def __init__(self, params, lr=0.02, momentum=0.95, overshoot=5, ns_steps=5, rank=0, world_size=1):
        self.rank = rank
        self.world_size = world_size
        defaults = dict(lr=lr, momentum=momentum, overshoot=overshoot, ns_steps=ns_steps)
        ...
        for group in self.param_groups:
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    g = g.lerp_(buf, 1 + overshoot - (overshoot / group["momentum"])) if overshoot else buf
